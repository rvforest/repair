...
- `credential-resolution`: Provider credential precedence, `pass` lookup behavior, local-provider exemptions, and secret-safe runtime failures.
...
- A credential-store abstraction and `pass` subprocess adapter are added using Node built-ins; no npm authentication dependency is required for the initial backend.
